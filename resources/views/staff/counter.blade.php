                    ...
                    <select name="showtime_id" class="mt-1 border-gray-300 rounded-md shadow-sm w-full p-3 text-lg"
                        ...
                        @foreach($showtimes as $st)
                            <option value="{{ $st->id }}" {{ (isset($selectedShowtime) && $selectedShowtime->id == $st->id) ? 'selected' : '' }}>
                                {{ \Carbon\Carbon::parse($st->start_time)->format('H:i') }} - {{ $st->movie->title }}
                                ({{ $st->room->name }})
                            </option>
                        @endforeach
                    </select>
                ...
                <div x-data="counterBooking()">
                    <div class="flex flex-col lg:flex-row gap-6 no-print">
                        ...
                        <div class="bg-white shadow rounded-lg p-6 lg:w-2/3">
                            <h3 class="text-xl font-bold mb-6 text-center border-b pb-2">Sơ đồ ghế:
                                {{ $selectedShowtime->room->name }}</h3>

                            <div class="mb-10 text-center">
                                <div class="w-2/3 mx-auto h-8 bg-gray-300 text-gray-600 font-bold leading-8 rounded-t-xl">MÀN HÌNH
                                </div>
                            </div>
                                    ...
                                    <div class="flex gap-2 items-center">
                                        ...
                                        <div class="flex gap-2">
                                            @foreach($rowSeats as $seat)
                                                ...
                                                @endphp
                                                <button @click="toggleSeat({{ json_encode($seat) }})"
                                                    class="h-10 rounded border-b-4 text-xs font-bold transition flex justify-center items-center {{ $seatType === 'couple' ? '' : 'w-10' }} {{ $seatClass }}"
                                                    style="{{ $seatType === 'couple' ? 'width: 5.5rem;' : '' }}"
                            ...
                            <div class="mt-8 flex flex-wrap justify-center gap-4 border-t pt-4">
                                <div class="flex items-center gap-2">
                                    <div class="w-6 h-6 bg-gray-200 border-b-4 border-gray-400 rounded"></div><span
                                        class="font-medium">Standard</span>
                                </div>
                                <div class="flex items-center gap-2">
                                    <div class="w-6 h-6 bg-amber-500 border-b-4 border-amber-600 rounded"></div><span
                                        class="font-medium">VIP</span>
                                </div>
                                <div class="flex items-center gap-2">
                                    <div class="w-12 h-6 bg-fuchsia-500 border-b-4 border-fuchsia-600 rounded"></div><span
                                        class="font-medium">Couple</span>
                                </div>
                                <div class="flex items-center gap-2">
                                    <div class="w-6 h-6 bg-red-500 border-b-4 border-red-700 rounded"></div><span
                                        class="font-medium">Đã đặt</span>
                                </div>
                                <div class="flex items-center gap-2">
                                    <div class="w-6 h-6 bg-green-500 border-b-4 border-green-700 rounded"></div><span
                                        class="font-medium">Đang chọn</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-white shadow rounded-lg p-6 lg:w-1/3 flex flex-col justify-between">
                            <div>
                                ...
                                <div class="mb-2"><strong>Thời gian:</strong>
                                    {{ \Carbon\Carbon::parse($selectedShowtime->start_time)->format('H:i d/m/Y') }}</div>
                                ...
                                <div class="flex flex-wrap gap-2 mb-4 min-h-[3rem]">
                                    <template x-for="seat in selectedSeats" :key="seat.seat_id">
                                        <span
                                            class="px-2 py-1 bg-green-100 text-green-800 rounded font-bold border border-green-300"
                                            x-text="seat.row + seat.number"></span>
                                    </template>
                                ...
                                <div class="mt-4 mb-2 border-t pt-4">
                                    <div class="flex justify-between items-center mb-2">
                                        ...
                                        <span class="text-sm font-semibold text-indigo-600"
                                            x-text="formatMoney(foodSubtotal) + ' ₫'">0 ₫</span>
                                    </div>
                                        ...
                                        <template x-for="item in snackItems" :key="item.id">
                                            <div class="flex items-center justify-between p-3 border rounded-lg bg-gray-50">
                                                <div>
                                                    ...
                                                    <div class="text-sm text-gray-500" x-text="formatMoney(item.price) + ' ₫'">
                                                    </div>
                                                </div>
                                                <div class="flex items-center gap-2">
                                                    <button @click="removeItem(item)" type="button"
                                                        class="w-8 h-8 rounded-full bg-white border border-gray-300 flex items-center justify-center text-gray-600 hover:bg-gray-100">−</button>
                                                    <span class="w-6 text-center font-bold"
                                                        x-text="getItemQuantity(item.id)"></span>
                                                    <button @click="addItem(item)" type="button"
                                                        class="w-8 h-8 rounded-full bg-indigo-50 border border-indigo-200 flex items-center justify-center text-indigo-600 hover:bg-indigo-100">+</button>
                                                </div>
                                            </div>
                                        </template>
                                    </div>
                                ...
                                <div class="flex justify-between items-end border-t pt-4 mt-4">
                                    ...
                                    <span class="text-3xl font-bold text-green-600" x-text="formatMoney(totalAmount) + ' ₫'">0
                                        ₫</span>
                                </div>
                            </div>
                            ...
                            <div class="mt-6">
                                ...
                                <button @click="processTicket" :disabled="selectedSeats.length === 0 || processing"
                    ...
                    <!-- Modal In Vé -->
                    <div x-show="ticketData"
                        class="fixed inset-0 z-50 overflow-y-auto bg-black bg-opacity-75 flex items-center justify-center p-4 no-print"
                        style="display: none;">
                        <div class="bg-white rounded-lg shadow-xl w-full max-w-sm p-6 text-center">
                            ...
                            <button @click="printTicket"
                                class="w-full py-3 bg-blue-600 text-white font-bold rounded-lg mb-3 hover:bg-blue-700 text-lg">
                                Xuất vé
                            </button>
                            <button @click="cancelBooking" :disabled="processing"
                                class="w-full py-3 bg-red-600 text-white font-bold rounded-lg mb-3 hover:bg-red-700 disabled:bg-red-300 text-lg">

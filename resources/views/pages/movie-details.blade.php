...
@section('content')
    ...
    <div class="relative w-full overflow-hidden bg-black/90 pt-8 pb-12 md:pt-16 md:pb-24">
        <!-- Blurred Background -->
        <div class="absolute inset-0 opacity-20 blur-3xl scale-110"
        ...
        <div class="container relative z-10 mx-auto px-4">
            <div class="flex flex-col md:flex-row gap-8 md:gap-12 items-center md:items-start">
                ...
                <div class="flex-1 text-center md:text-left text-white space-y-4 mt-4 md:mt-0 font-body">
                    <h1 class="text-4xl md:text-5xl lg:text-6xl font-marquee tracking-tight">{{ $movie->title }}</h1>

                    <div class="flex flex-wrap items-center justify-center md:justify-start gap-4">
                        <span
                            class="bg-primary text-white text-sm px-3 py-1 rounded font-semibold">{{ $movie->age_rating }}</span>
                        ...
                        @foreach($movie->tags as $tag)
                            <span
                                class="border border-white/30 bg-white/5 text-white/80 text-sm px-3 py-1 rounded">{{ $tag->name }}</span>
                        @endforeach
                    ...
                    {{-- Thông tin chi tiết — không còn khung, chỉ đường kẻ mảnh phân cách nhẹ --}}
                    <div class="text-left pt-3 border-t border-white/10">
                        <h2 class="text-xl font-marquee text-white">Thông tin chi tiết</h2>
                        <dl class="mt-2 space-y-2 text-sm text-slate-300">
                            <div>
                                <dt class="font-semibold text-slate-400">Đạo diễn</dt>
                                <dd class="mt-0.5 text-white">{{ $movie->director ?: 'Đang cập nhật' }}</dd>
                            </div>
                            <div>
                                <dt class="font-semibold text-slate-400">Diễn viên</dt>
                                <dd class="mt-0.5 text-white">{{ $movie->actors ?: 'Đang cập nhật' }}</dd>
                            </div>
                        </dl>
                    </div>

                    {{-- Nội dung phim — không còn khung --}}
                    <div class="text-left pt-3 border-t border-white/10">
                        <h2 class="text-xl font-marquee text-white">Nội dung phim</h2>
                        <p class="mt-2 whitespace-pre-line text-slate-300 leading-7 max-w-3xl">
    ...
    <div class="container mx-auto px-4 py-12 font-body">
        <h2 class="text-3xl font-marquee mb-8 flex items-center gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none"
                stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                class="lucide lucide-calendar text-primary">
                    ...
                    <div class="bg-dark-card border border-dark-border rounded-xl p-6 liquid-glass">
                        <div class="flex flex-wrap gap-4">
                            @foreach($showtimes as $st)
                                @if(\Carbon\Carbon::now()->greaterThan(\Carbon\Carbon::parse($st->start_time)))
                                    <span class="flex flex-col gap-1 items-center justify-center border border-slate-700/50 text-slate-500 bg-slate-800/30 py-4 px-6 rounded-xl cursor-not-allowed opacity-50">
                                        <span class="font-bold text-xl">{{ \Carbon\Carbon::parse($st->start_time)->format('H:i') }}</span>
                                        <span class="text-xs text-slate-500">{{ $st->room->name }}</span>
                                    </span>
                                @else
                                    <a href="{{ url('/showtimes/' . $st->id . '/seats') }}"
                                        class="flex flex-col gap-1 items-center justify-center border border-slate-700 hover:border-primary hover:text-primary bg-slate-800 hover:bg-slate-800/50 transition-colors py-4 px-6 rounded-xl">
                                        <span class="font-bold text-xl">{{ \Carbon\Carbon::parse($st->start_time)->format('H:i') }}</span>

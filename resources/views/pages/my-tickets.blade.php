...
<div class="container mx-auto px-4 py-8 max-w-4xl">
    <h1 class="text-2xl font-bold text-white mb-6 border-b border-gray-700 pb-4">Lịch sử đặt vé của tôi</h1>
    ...
    @if($bookings->isEmpty())
        <div class="bg-gray-800 rounded-lg p-8 text-center border border-gray-700">
            ...
            <a href="{{ route('home') }}" class="inline-block bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-6 rounded transition">
                Xem lịch chiếu ngay
            </a>
        </div>
    @else
        <div class="grid gap-6">
            @foreach($bookings as $booking)
                <div class="bg-gray-800 rounded-lg shadow-lg overflow-hidden border border-gray-700 hover:border-gray-600 transition flex flex-col md:flex-row">
                    ...
                    <div class="p-5 flex-grow flex flex-col justify-between">
                        <div>
                            ...
                            <div class="text-sm text-gray-400 grid grid-cols-2 gap-y-2 mt-4">
                                ...
                                <div><span class="text-gray-500">Thời gian:</span> <span class="font-semibold text-red-400">{{ \Carbon\Carbon::parse($booking->showtime->start_time)->format('H:i - d/m/Y') }}</span></div>
                                <div><span class="text-gray-500">Tổng tiền:</span> <span class="font-semibold text-yellow-500">{{ number_format($booking->total_amount, 0, ',', '.') }} ₫</span></div>
                            </div>
                        </div>
